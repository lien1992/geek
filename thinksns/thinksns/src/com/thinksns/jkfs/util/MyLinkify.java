...
public class MyLinkify {
	...
	public static final boolean addLinks(Spannable text, int mask) {
		...
		if ((mask & WEB_URLS) != 0) {
			gatherLinks(links, text, MyPatterns.WEB_URL, new String[] {
	...
	public static final SpannableString addJUstHighLightLinks(
			CharSequence text, int mask) {
		if (mask == 0) {
	...
	public static final void addJustHighLightLinks(TextView text, Pattern p,
	...
	public static final SpannableString getJustHighLightLinks(CharSequence s,
			Pattern p, String scheme, MatchFilter matchFilter,
			TransformFilter transformFilter) {
	...
	private static final String makeUrl(String url, String[] prefixes,
		...
		for (int i = 0; i < prefixes.length; i++) {
			if (url
					.regionMatches(true, 0, prefixes[i], 0, prefixes[i]
							.length())) {
				hasPrefix = true;

				// Fix capitalization if necessary
				if (!url.regionMatches(false, 0, prefixes[i], 0, prefixes[i]
						.length())) {
